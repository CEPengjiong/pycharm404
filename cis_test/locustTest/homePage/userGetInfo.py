...
class WebsiteTasks(TaskSet):
    ...
    def userGerInfo(self):
        ...
        # getInfo接口 获取用户信息
        url ='/prod-api/clinic/menu/getRouters?_t=' + str(api_congfig._t)
        res_text = self.client.get(url, name='/user/getInfo', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, 'getInfo接口获取失败\n结果------------%s' % res_text.text
