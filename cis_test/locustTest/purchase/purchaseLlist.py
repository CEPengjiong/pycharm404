...
class WebsiteTasks(TaskSet):
    ...
    def purchaseList(self):
        ...
        url ='/prod-api/clinic/medicine/purchase/list?_t=' + str(api_congfig._t) + '&pageNum=1&pageSize=10&state=&beginTime=&endTime='
        res_text = self.client.get(url, name='/medicine/purchase/list', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/medicine/purchase/list接口获取失败\n结果------------%s' % res_text.text
