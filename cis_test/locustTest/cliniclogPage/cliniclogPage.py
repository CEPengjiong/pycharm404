...
class WebsiteTasks(TaskSet):
    ...
    def cliniclogPage(self):
        ...
        url ='/prod-api/clinic/cliniclog/page?_t=' + str(api_congfig._t) + '&pageNum=1&pageSize=10&beginTime=&endTime='
        res_text = self.client.get(url, name='/cliniclog/page', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/cliniclog/page接口获取失败\n结果------------%s' % res_text.text
