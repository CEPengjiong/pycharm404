...
class WebsiteTasks(TaskSet):
    ...
    def chargeStatistics(self):
        ...
        url ='/prod-api/clinic/tchargerecord/chargeStatistics?_t=' + str(api_congfig._t) + '&beginTime=2019-10-01&endTime=2021-10-31'
        res_text = self.client.get(url, name='/tchargerecord/chargeStatistics', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/tchargerecord/chargeStatistics接口获取失败\n结果------------%s' % res_text.text
