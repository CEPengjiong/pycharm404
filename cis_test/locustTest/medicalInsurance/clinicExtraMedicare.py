...
class WebsiteTasks(TaskSet):
    ...
    def clinicExtraMedicare(self):
        ...
        url ='/prod-api/clinic/clinicExtra/medicare?_t=' + str(api_congfig._t) + '&pageNum=1&pageSize=10&name='
        res_text = self.client.get(url, name='/clinicExtra/medicare', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/clinicExtra/medicare接口获取失败\n结果------------%s' % res_text.text
