...
class WebsiteTasks(TaskSet):
    ...
    def centerList(self):
        ...
        url ='/prod-api/clinic/medicine/info/centerList?_t=' + str(api_congfig._t) + '&pageNum=1&pageSize=10&drugType=0&drugName=a'
        res_text = self.client.get(url, name='/medicine/info/centerList', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/medicine/info/centerList接口获取失败\n结果------------%s' % res_text.text
