"""
description: 满足定制化需求，读取excel文件数据，逐行读取
time:
author: chenling
"""
import json
...
def __get_cell_value(xlUtils:XlrdUtils, sheet_index_or_name, row_num,*cols_index):
    if len(cols_index) == 0:
        ...
        EXCEPET_RESULT = json.loads(xlUtils.get_cell_value(sheet_index_or_name, row_num, 9))
        return [case_num, moudle_name, case_name, request_method, request_url,
                ...
                except_result_desc, trans_data(EXCEPET_RESULT)]
    else:
        row_data = []
