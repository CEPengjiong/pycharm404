...
class TestClinicDaily:
    curPath = os.path.abspath(os.path.dirname(__file__))
    rootPath = os.path.split(curPath)[0]
    sys.path.append(rootPath)
    data_path = rootPath + r"\data\clinicDaily.xls"
    ...
    @allure.description("收入详情查询-收入占比查询")
    ...
    def test_clinicDaily_case(self, moudle_name, case_name, request_method, request_url, req_data, code, exc_data, clinic_login):
        # logging.info("请求的header：{}".format(api_config.request_header))

        res_text = ApiFactory.get_clinicDaily().clinicDaily(moudle_name=moudle_name, case_name=case_name, request_method=request_method, url=request_url, data=req_data)
