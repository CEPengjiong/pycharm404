...
class WebsiteTasks(TaskSet):
    ...
    def chargeManagementPage(self):
        ...
        url ='/prod-api/clinic/tvisitorder/chargeManagementPage?_t=' + str(api_congfig._t) + '&pageNum=1&pageSize=20&status=2'
        res_text = self.client.get(url, name='chargeManagementPage', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/tvisitorder/chargeManagementPage接口获取失败\n结果------------%s' % res_text.text
