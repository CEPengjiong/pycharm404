...
class WebsiteTasks(TaskSet):
    ...
    def selectMedicineStock(self):
        ...
        url ='/prod-api/clinic/sale/drugs/saleOrders?_t=' + str(api_congfig._t) + '&pageNum=1&pageSize=10'
        res_text = self.client.post(url, name='/sale/drugs/saleOrders', headers=self.head, data=self.body, verify=False)
        assert '操作成功' in res_text.text, '/sale/drugs/saleOrders接口获取失败\n结果------------%s' % res_text.text
