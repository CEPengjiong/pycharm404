...
class WebsiteTasks(TaskSet):
    ...
    def patientRegisterPage(self):
        ...
        # /clinicconfig/info接口 获取诊所配置信息
        url ='/prod-api/clinic/patientregister/page' + str(api_congfig._t) + '&pageNum=1&pageSize=10&beginTime=&endTime='
        res_text = self.client.get(url, name='/patientregister/page', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/patientregister/page接口获取失败\n结果------------%s' % res_text.text
