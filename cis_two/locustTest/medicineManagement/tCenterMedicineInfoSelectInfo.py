...
class WebsiteTasks(TaskSet):
    ...
    def selectInfo(self):
        ...
        url ='/clinic/tcentermedicineinfo/selectInfo?_t=' + str(api_congfig._t) + '&name=n&type=1'
        res_text = self.client.get(url, name='/tcentermedicineinfo/selectInfo', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/tcentermedicineinfo/selectInfo接口获取失败\n结果------------%s' % res_text.text
