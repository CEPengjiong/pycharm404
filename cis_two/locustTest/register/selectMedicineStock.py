...
'''
开启脚本  点击下方Terminal打开控制台，cd 到unicom文件目录下，执行locust -f locust.py
...
class WebsiteTasks(TaskSet):
    ...
    def selectMedicineStock(self):
        ...
        url ='/prod-api/clinic/medicine/info/selectMedicineStock?_t=' + str(api_congfig._t)
        res_text = self.client.post(url, name='selectMedicineStock', headers=self.head, data=self.body, verify=False)
        assert '操作成功' in res_text.text, '/medicine/info/selectMedicineStock接口获取失败\n结果------------%s' % res_text.text
