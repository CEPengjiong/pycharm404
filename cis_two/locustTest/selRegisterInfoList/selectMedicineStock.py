...
class WebsiteTasks(TaskSet):
    ...
    def selectMedicineStock(self):
        ...
        url ='/prod-api/clinic/patientregister/selRegisterInfoList?_t=' + str(api_congfig._t)
        res_text = self.client.get(url, name='selRegisterInfoList', headers=self.head, verify=False)
        assert '操作成功' in res_text.text, '/patientregister/selRegisterInfoList接口获取失败\n结果------------%s' % res_text.text
